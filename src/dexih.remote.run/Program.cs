...
");
                ...
                while (upgrade == 20)
                {
                    ...
                    {
                        Console.WriteLine("Updating the binaries");
                        var updateDirectory = Path.GetFullPath("update");
                        var tempDirectory = Path.Combine(Path.GetTempPath(), Path.GetRandomFileName());
                        var backupDirectory = "dexih_remote_" + DateTime.Now.ToString("yyyyMMddHHmmssmi");

                        Directory.Move(updateDirectory, tempDirectory);
                        Directory.Move(directory, backupDirectory);
                        Directory.Move(tempDirectory, directory);

                        Console.WriteLine("Previous version is backed up to " + backupDirectory);
                    }
                    else
                    {
