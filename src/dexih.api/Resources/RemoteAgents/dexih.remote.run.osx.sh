...
install() 
{
        if ! [ -x "$(command -v $1)" ]; then
                error_exit "$1 command is not insatlled.  Manually install this and rerun the script."
        fi
}
...
do
    ...
    if [ "${LATEST_VERSION}" != "${LOCAL_VERSION}" ]  || [ ! -d ${DIRECTORY}  ]; then
        ...
        unzip -q ${LATEST_BINARY} -d ${DIRECTORY} || error_exit "Could not unzip the ${LATEST_BINARY}, check the file is a valid zip file."
        # rm ${LATEST_BINARY}

        echo ${LATEST_VERSION} > local_version.txt || error_exit "Could not update hte file local_version.txt, check the file permissions."

        chmod a+x ./${DIRECTORY}/dexih.remote || error_exit "Could not update hte permissions on the ${DIRECTORY}/dexih.remote file."
